...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 75, 'input_size': (1, 128, 496), 'pool_size': None,
...
class PoolFormer(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None, in_channel=1,
                 mlp_ratios=None, downsamples=None,
                 pool_size=3,
                 norm_layer=GroupNorm, act_layer=nn.GELU,
                 num_classes=75,
...
@register_model
def poolformer_s36(num_classes=75, pretrained=False, in_channel=1,  emb=64 ,**kwargs):
    """
    ...
    downsamples = [True, True, True, True]
    model = PoolFormer(
        layers, embed_dims=embed_dims, num_classes=num_classes, in_channel=in_channel,
        mlp_ratios=mlp_ratios, downsamples=downsamples, layer_scale_init_value=1e-6,
        **kwargs)
    model.default_cfg = default_cfgs['poolformer_s']
    if pretrained:
...
if __name__ == '__main__':
    ...
    img = torch.randn(1, 1, 16 * 8, 512 - 16)
    print(model(img).shape)  # (1, 75)
    pred = model(img)